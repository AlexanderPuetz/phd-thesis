...
class Cell:
    ...
    def __init__(self, x, y, state):
        ...
        self.y = y
        self.state = state
        # self.neighbors = self.how_many_neighbors()
            ...
            if mode == "pbc": # loop at borders of the lattice
                ...
                if neigh_x <= min_x:
                    ...
                    for y in range(min_y, max_y + 1): 
                        cell = str(f"{neigh_x}|{neigh_y}")
                        cells[cell] = Cell(min_x, y, 0)
                elif neigh_x >= max_x:
                    max_x = neigh_x
                    for y in range(min_y, max_y + 1): 
                        cell = str(f"{neigh_x}|{neigh_y}")
                        cells[cell] = Cell(max_x, y, 0)
                elif neigh_y <= min_y:
                    min_y = neigh_y
                    for x in range(min_x, max_x + 1): 
                        cell = str(f"{neigh_x}|{neigh_y}")
                        cells[cell] = Cell(x, min_y, 0)
                elif neigh_y >= max_y:
                    max_y = neigh_y
                    for x in range(min_x, max_x + 1): 
                        cell = str(f"{neigh_x}|{neigh_y}")
                        cells[cell] = Cell(x, max_y, 0)
...
def create_visible_cells():
    for x,y in ((x,y) for x in range(-1, size[0]+2) for y in range(-1, size[1]+2)):
        cell = str(f"{x}|{y}")
...
def decide_fate(x,y):
    ...
    if n == bsc[0]: changes.append([x, y, 1])
    elif n < bsc[1] or n > bsc[2]: changes.append([x, y, 0])
